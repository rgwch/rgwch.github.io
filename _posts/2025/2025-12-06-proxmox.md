...
Für einen Hypervisor kann man viel Geld ausgeben, wenn man ihn zum Beispiel bei Broadcom lizensiert. Man kann aber auch einen OpenSource-Hypervisor erst mal kostenlos installieren und nutzen.
...
### Virtual Machines (VM): 
Die sind verglechbar mit Docker-Containern. Eine VM kann ein beliebiges Betrienbssystem enthalten, sofern es auf dem X86-Prozessor des Hosts ausführbar ist. Es braucht aber auch entsprechend Platz und Ressourcen. EIne VM erstellt man mit dem blauen Button "Create VM" rechts oben. Danach muss man einen Namen für die Maschine eingeben und dann ein Start-Image auswählen. Das kann entweder eine CD/DVD im physischen Laufwerk sein, oder eine ISO-Datei. Letztere muss man allerdings zuerst hochladen. Die Option dafür ist ein wenig versteckt:
Gehen Sie in der Dropbox links oben auf "Folöder view", klappen Sie dann "Storage" auf. Wählen Sie das lokale Laufwerk und dort dann "ISO-Images":
...
Bei den weiteren Otionen können Sie meist die Voreinstellung belassen. Wenn in der Auswahl vorgesehen, können Sie mit "virt..." Geräten eine etwas bessere Performance erreichen: Paravirtualisierte Hardware nutzt die Treiber des Host-Betriebssystems, statt sie zu emulieren, und gewinnt dadurch Leistung auf Kosten der dann nicht mehr so vollständigen Isolierung vom Hostsystem. 
Am Ende können Sie die Maschine starten und sich sofort mit der Konsole verbinden:

![Konsole](/images/freebsd_console.png)

Von hier aus können Sie z.B. einen openssh-server installieren, um sich auch unabhöngig von Proxmox mit dieser virtuellen Maschine verbinden zu können, oder auch einen grafischen Deskop installieren, auf den Sie sich mit VNC, X2Go oder xrdp verbinden können.
