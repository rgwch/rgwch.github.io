...
Ein umfangreiches Programm installieren, ohne den eigenen PC zu zerschiessen? Ein neues Betriebssystem einfach mal ausprobieren, ohne die Funktion des aktuellen Systems zu riskieren? Mehrere Dienste anbieten, die sich gegenseitig nicht beeinflussen?
...
Für einen Hypervisor kann man viel Geld ausgeben, wenn man ihn zum Beispiel bei Broadcom lizensiert. Man kann aber auch einen OpenSource-Hypervisor auf Linux-Basis erst mal kostenlos installieren und nutzen.
...
apt install sudo xfce4 dbus-x11 xrdp
adduser IhrUserName
...
```bash
apt update && apt upgrade -y
apt install openjdk-21-jdk-headless maven git sudo
...
Dann den Browser auf `http://DieserServer:8080 richten und den Anweisungen folgen. Das Initialpasswort wurde am Ende im Terminal angezeigt. Sie benötigen es nur einmal.
Wenn Sie nicht sicher sind, wie Sie Jenkins einrichten wollen, fangen sie mit "install suggested plugins" an. Das wird zwar ziemlich lang dauern, aber danach können Sie fast alles machen.


## Tipps:

### Diskgrösse verändern: 

` pct resize <id> rootfs <newsize>`
oder im UI unter der entsprechenden VM und "Ressources"
...
### Samba-Share einbinden

* im host /etc/fstab: `//server/share /mnt/sharename cifs user=user,password=pwd,uid=100000,gid=100000,file_mode=0666,dir_mode=0777,nofail 0 0`
* In den LXCs: `nano /etc/pve/lxc/XXX.conf` und dort die Zeile `mp0: /mnt/sharename,mp=/mnt/lxc_sharename` eintragen
